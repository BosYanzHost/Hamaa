...
async def check_role(user_id, required_level):
...
def PY_SELLER(func):
    """Akses untuk SELLER ke atas (SELLER, ADMIN, TK, CEO, ALLROLE, PT, OWNER)"""
    async def wrapper(client, message):
        if not await check_role(message.from_user.id, "SELLER"):
            return await message.reply("❌ Perintah ini hanya untuk SELLER ke atas!")
        return await func(client, message)
    return wrapper

def PY_ADMIN(func):
    """Akses untuk ADMIN ke atas (ADMIN, TK, CEO, ALLROLE, PT, OWNER)"""
    async def wrapper(client, message):
...
def PY_TK(func):
    """Akses untuk TK ke atas (TK, CEO, ALLROLE, PT, OWNER)"""
    async def wrapper(client, message):
        if not await check_role(message.from_user.id, "TK"):
            return await message.reply("❌ Perintah ini hanya untuk TK ke atas!")
        return await func(client, message)
    return wrapper

def PY_CEO(func):
    """Akses untuk CEO ke atas (CEO, ALLROLE, PT, OWNER)"""
    async def wrapper(client, message):
        if not await check_role(message.from_user.id, "CEO"):
            return await message.reply("❌ Perintah ini hanya untuk CEO ke atas!")
        return await func(client, message)
    return wrapper

def PY_ALLROLE(func):
    """Akses untuk ALLROLE ke atas (ALLROLE, PT, OWNER)"""
    async def wrapper(client, message):
        if not await check_role(message.from_user.id, "ALLROLE"):
            return await message.reply("❌ Perintah ini hanya untuk ALLROLE ke atas!")
        return await func(client, message)
    return wrapper

def PY_PT(func):
    """Akses untuk PT ke atas (PT, OWNER)"""
    async def wrapper(client, message):
        if not await check_role(message.from_user.id, "PT"):
