...
ᚗ <code>{0}cadp [nama]</code>
⊶ Membuat admin panel (root admin)
...
"""
        ...
        await processing.edit_text(f"{ggl} Gagal: {str(e)[:200]}")
# ============================================
# COMMAND CADP (CREATE ADMIN PANEL)
# ============================================

@PY.UBOT("cadp")
async def cadp_command(client, message):
    ggl = await EMO.GAGAL(client)
    sks = await EMO.BERHASIL(client)
    prs = await EMO.PROSES(client)
    ...
    if len(args) < 2:
        await message.reply_text(
            f"<blockquote><b>❌ FORMAT CADP</b>\n\n"
            f"<b>Contoh:</b>\n"
            f"<code>.cadp joko</code>\n"
            f"<code>.cadp admin1</code>\n"
            f"<code>.cadp budi</code></blockquote>"
        )
        return
    ...
    processing = await message.reply_text(f"{prs} Membuat admin panel untuk {username}...")
    ...
    try:
        # Generate data
        password = username + '117'  # sesuai contoh
        email = f"{username}@gmail.com"
        
        # Cek egg valid (opsional, jika gagal lanjut)
        egg_data = None
        try:
            egg_data = await get_valid_egg(selected_server, DEFAULT_EGGS.get("1gb", [15]))
        except Exception as e:
            print(f"Egg check skipped: {e}")
        
        # CREATE USER ADMIN
        await processing.edit_text(f"{prs} Membuat admin user {username} di panel...")
        ...
        if user_response.status_code not in [200, 201]:
            error_data = user_response.json()
            error_msg = error_data.get("errors", [{}])[0].get("detail", "Unknown error")
            raise Exception(f"Gagal buat admin user: {error_msg}")
        ...
        # Kirim detail ke CHAT DIMANA USER NGETIK
        egg_text = f"• Egg ID: {egg_data['eggId']}" if egg_data else ""
...
<b>🌐 Server:</b> {selected_server['name']}
<b>🔗 Akses Login:</b> {selected_server['domain']}"

<b>📌 Rules Admin Panel</b>
• No Rusuh Panel
• Simpan Data Akun
• No Share Free</blockquote>
"""
        )
