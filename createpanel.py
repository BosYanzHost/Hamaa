...
COOLDOWN_FILE = "ptero_cooldown.json"
...
PANEL_IMAGE = "https://telegra.ph/file/9b3e0f8b7c5d8f9a1e2b3.jpg"  # Ganti dengan URL gambar lo
...
def check_cooldown(user_id, cmd):
    """Cek cooldown untuk command tertentu"""
    cooldowns = load_json(COOLDOWN_FILE, {})
    key = f"{user_id}_{cmd}"
    
    if key in cooldowns:
        last_time = cooldowns[key]
        elapsed = time.time() - last_time
        if elapsed < 180:  # 3 menit cooldown
            return int(180 - elapsed)
    return 0

def set_cooldown(user_id, cmd):
    """Set cooldown untuk command"""
    cooldowns = load_json(COOLDOWN_FILE, {})
    key = f"{user_id}_{cmd}"
...
# ============================================
# COMMAND SETTING SERVER (OWNER ONLY)
# ============================================

@PY.UBOT("setpanel")
async def setpanel_command(client, message):
    """Command untuk menambah/mengatur server panel"""
    ggl = await EMO.GAGAL(client)
    sks = await EMO.BERHASIL(client)
    
    # Cek apakah user adalah owner
    if message.from_user.id not in OWNER_ID:
        await message.reply_text(f"{ggl} Perintah ini hanya untuk owner!")
        return
    ...
    if len(args) < 2:
        await message.reply_text(
            ...
            f"<b>Contoh:</b>\n"
            f"<code>.setpanel add server1 https://panel.domain.com ptau_xxx ptc_xxx</code></blockquote>"
        )
        return
    ...
    if action == "add":
        ...
        await message.reply_text(
            ...
            f"🔑 <b>PLTC:</b> <code>{pltc[:15]}...</code>\n\n"
            f"📌 Simpan ID server untuk referensi</blockquote>"
        )
...
async def unli_command(client, message):
    ...
    # Cek format - LANGSUNG NAMA AJA, TANPA ID!
    args = message.text.split()
    if len(args) < 2:
        await message.reply_text(
    ...
    # Cek cooldown
    cooldown = check_cooldown(message.from_user.id, "unli")
    if cooldown > 0:
        await message.reply_text(f"⏳ Tunggu {cooldown} detik sebelum bisa pakai command .unli lagi!")
        return
    ...
    try:
        ...
        # Set cooldown
        set_cooldown(message.from_user.id, "unli")
        
        # Kirim detail ke CHAT DIMANA USER NGETIK (BUKAN KE USER LAIN)
        memory_text = "Unlimited"
        disk_text = "Unlimited"
...
async def panel_command(client, message):
    ...
    # Parse command (format: .panel [tipe] [nama] - TANPA ID!)
    args = message.text.split()
    if len(args) < 3:
        await message.reply_text(
    ...
    # Cek cooldown
    cooldown = check_cooldown(message.from_user.id, cmd_type)
    if cooldown > 0:
        await message.reply_text(f"⏳ Tunggu {cooldown} detik sebelum bisa pakai command .{cmd_type} lagi!")
        return
    ...
    try:
        ...
        # Set cooldown
        set_cooldown(message.from_user.id, cmd_type)
